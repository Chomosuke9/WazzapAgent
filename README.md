...
`start.sh` resolves the bridge's absolute Python executable without leaving its
virtualenv, then exports it as `PY_BIN`; Node uses that same interpreter for
Python-backed tools such as SpotDL. Dependency installation remains an explicit
setup/deployment step.
