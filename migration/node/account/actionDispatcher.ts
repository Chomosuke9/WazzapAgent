...
async function routeAction(
  ...
  if (type === 'mark_read') {
    await deps.markChatRead(ctx, payload);
    return;
  }
  ...
  if (type === 'run_command') {
    ...
    let result: any;
    try {
      result = await deps.dispatchRunCommand(ctx, payload);
    } catch (err) {
      const detail = actionErrorDetail(err);
      emitActionAck(entry, {
        requestId,
  ...
  if (type === 'send_presence') {
    await deps.sendPresence(ctx, payload);
    return;
  }
  ...
  if (type === 'relay_lottie_sticker') {
    ...
    try {
      const result = await deps.sendLottieSticker(ctx, chatId, lottiePayload, replyTo);
      emitActionAck(entry, {
  ...
  if (type === 'send_quiz') {
    ...
    try {
      ...
      const { MAX_QUIZ_IDS } = await import('../caches.js');
  ...
  if (type === 'send_buttons') {
    ...
    const result = await deps.sendNativeFlow(sock as any, payload.chatId, payload.text || '', nativeButtons, { footer: payload.footer });
  ...
  if (type === 'send_carousel') {
    ...
    const result = await deps.sendCarousel(sock as any, payload.chatId, cards, { text: payload.text });
