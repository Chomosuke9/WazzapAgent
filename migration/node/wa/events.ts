...
async function emitGroupJoinContextEvent(ctx: AccountContext, {
  ...
  if (config.requireActivation && !isChatActivated(chatId)) return;
  ...
  forwardIncoming(registry.getOrCreate(ctx.folderPath), payload as unknown as WhatsAppMessagePayload);
}
