...
def build_session(account, index: int, base_webhook_port: int = SUBAGENT_WEBHOOK_PORT) -> AgentSession:
...
async def main():
  ...
  for index, account in enumerate(accounts):
    ...
    logger.info(
      ...
      index, account.folder_path, account.node_url, session.subagent_webhook._port,
    )
  ...
  try:
    ...
    await asyncio.gather(
      *(session.run(account.node_url, stop_event) for account, session in sessions)
    )
  finally:
