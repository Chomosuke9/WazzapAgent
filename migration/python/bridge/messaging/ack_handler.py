"""Step 29 — re-homed ``action_ack`` handler (D3 option b).

The old ``async for raw in ws`` loop in ``bridge.main`` handled
``action_ack`` to:

  * hydrate a provisional history entry (``context_msg_id="pending"`` -> the
    real 6-digit id) via
    :func:`bridge.messaging.processing._hydrate_provisional_context_id_from_ack`,
  * record sub-agent attachment file paths into ``media_paths_by_chat`` via
    :func:`bridge.messaging.processing._extract_all_send_ack_entries`, and
  * append the synthetic ``"Command X executed/failed"`` line for
    ``run_command`` acks.

Step 28 removed that loop and re-homes the work onto the WaSocket SDK's
re-emitted ``action_ack`` event. The logic below is preserved VERBATIM from the
old loop; it lives in this small, dependency-light module (importing only
``processing`` + ``history`` + ``log``, NOT the heavyweight ``bridge.main``) so
it can be unit-tested by driving it directly with a synthetic ``AckResult``.

``handle_action_ack`` reads the SDK's :class:`wasocket.protocol.AckResult`
(``ack.request_id`` / ``ack.action`` / ``ack.ok`` / ``ack.detail`` /
``ack.result`` / ``ack.code``) and rebuilds the legacy payload dict the original
loop body operated on, so the moved body is byte-for-byte identical.
"""
