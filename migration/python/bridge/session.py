...
# ---------------------------------------------------------------------------
# Sticker command helpers (stateless / parameterised — unchanged from main.py)
# ---------------------------------------------------------------------------
...
class AgentSession:
  ...
  def __init__(self, sock, *, webhook_port: int | None = None, webhook_url: str | None = None) -> None:
    self.sock = sock
    ...
    self.recent_reply_signatures_by_chat: Dict[str, Deque[tuple[int, str]]] = defaultdict(deque)
    ...
    # store file paths into ``media_paths_by_chat`` under their real
    # contextMsgId once the gateway acknowledges the send. Uses OrderedDict
    # for LRU eviction to prevent unbounded growth if acks are lost.
    ...
    # Keyed by ``request_id``; value is ``(chat_id, command_text)``.
    ...
    # constraint when N accounts run in one process.)
  ...
  def register(self) -> None:
    """Wire the WaSocket event handlers for this session (Steps 28/29 bodies).

    Delegates to the module-level :func:`_register_handlers`, which binds
    ``self``'s per-account state to locals of the same name and defines the
    handler closures at the original ``handle_socket`` indentation so the
    moved bodies are byte-for-byte identical. Because the bound containers are
    mutable and shared by reference, every mutation lands on ``self``.
    """
  ...
  def tenant_db(self):
    """Context manager binding this session's per-tenant DB directory.

    Mirrors what :meth:`run` does for the live event loop, exposed so callers
    (and tests) can perform DB operations "as this session" — i.e. routed to
    ``<folder_path>/db`` — outside the running pump.
    """
    return db_tenant_db_context(self.folder_path)


def _register_handlers(session) -> None:
  """Bind ``session``'s state to locals and define + wire the WaSocket event
  handlers. Defined at module scope so the handler closures sit at the exact
  indentation of the original ``handle_socket`` body and are copied verbatim.
  """
  ws = session.sock
  # Bind instance state to locals so the moved closure bodies are verbatim.
  ...
  recent_reply_signatures_by_chat = session.recent_reply_signatures_by_chat
  ...
  # in the ``finally`` block of ``run`` when the gateway disconnects so a
  ...
  # Stash so run() can set/clear the queue handler around the connection.
  session._queue_handler = _on_subagent_queue_event
  ...
  async def process_message_batch(payloads: list[dict]):
    ...
    for payload in non_empty_payloads:
      ...
      if cmd_name == "dump":
        ...
        try:
          from .llm.llm2 import _render_system_prompt, _load_system_prompt, _group_description_block, _chat_state_header, _current_date_str
          ...
          from bridge.llm.llm2 import _render_system_prompt, _load_system_prompt, _group_description_block, _chat_state_header, _current_date_str  # type: ignore
...
context block from ``SubTaskTracker.format_context``).
  ...
  # --- WaSocket event handlers (Step 28 transport flip) ---
  # The former `async for raw in ws` dispatch loop is re-homed onto WaSocket
  # event handlers. `_dispatch_event` carries the control-event + incoming-
  # message branch bodies VERBATIM (only `continue` -> `return`). The
  # action_ack hydration logic (Step 29) lives in the module-level
  # `_handle_action_ack` so it is unit-testable with a synthetic AckResult.
  ...
  async def _on_status(status):
    # The old loop ignored whatsapp_status (it fell through to the
    # incoming_message guard); preserve that no-op. Status values are now
    # normalized to open|connecting|close.
  ...
  @ws.on("action_ack")
  async def _on_action_ack(ack):
    await _handle_action_ack(
      ack,
      per_chat=per_chat,
  ...
  async def _on_send_ack(ack):
    ...
    logger.debug("Gateway send_ack: %s", getattr(ack, "request_id", None))

  @ws.on("clear_history")
  async def _on_clear_history(evt):
    await _dispatch_event(evt)
  ...
  async def _on_message(msg):
    ...
    await _dispatch_event({"type": "incoming_message", "payload": msg.raw})
