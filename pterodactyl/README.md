...
| **SpotDL is missing or says `Python versions to be found at the same time`** | Restart after updating. The bootstrap/start supervisor installs SpotDL into the bridge's Python environment, and the gateway invokes it through the same `PY_BIN` (`python -m spotdl`) instead of an ambiguous `spotdl`/pyenv shim. |
