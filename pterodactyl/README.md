...
| **SpotDL is missing or says `Python versions to be found at the same time`** | The bootstrap installs SpotDL into its managed Python environment. `start.sh` exports that interpreter's absolute path as `PY_BIN`, and the gateway invokes `PY_BIN -m spotdl` instead of an ambiguous `spotdl`/pyenv shim. |
