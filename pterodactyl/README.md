...
| **SpotDL says `Python versions to be found at the same time`** | Restart after updating. The bootstrap installs SpotDL into the portable Python environment, and the gateway invokes it through the same `PY_BIN` as the bridge (`python -m spotdl`) instead of an ambiguous `spotdl`/pyenv shim. |
