...
const STICKERS_DB_PATH = process.env.BOT_STICKERS_DB_PATH
  ...
  || path.join(config.dataDir, 'stickers.db');

// Directory where uploaded sticker files are stored persistently
const STICKER_UPLOAD_DIR = process.env.STICKER_UPLOAD_DIR
  || path.join(config.dataDir, 'stickers_user');
 ...
 */
function isLottieSticker(msgObj) {
 ...
 */
function serializeLottiePayload(msgObj, stickerContent) {
 ...
 */
async function downloadStickerToTemp(stickerContent, messageId) {
 ...
 */
async function persistStickerFile(tempPath, chatId, name) {
...
function upsertWebpSticker(chatId, name, filePath, addedBy) {
  const db = getDb();
...
function upsertLottieSticker(chatId, name, lottiePayloadJson, addedBy) {
  const db = getDb();
  const existing = db.prepare(
    'SELECT id FROM stickers WHERE chat_id = ? AND name = ?',
  ).get(chatId, name);
...
async function handleAddSticker({
  ...
  let stickerContent = null;       // the stickerMessage proto object
  let sourceMsgObj = null;         // the raw message object containing the sticker
  let messageIdForFile = msg.key?.id || randomUUID();

  /**
   * Extract the stickerMessage content + the source msgObj from any wrapper.
   */
  function extractSticker(msgObj) {
  ...
  if (!stickerContent) {
    const ctx = innerMessage?.extendedTextMessage?.contextInfo
      ...
      || innerMessage?.lottieStickerMessage?.message?.stickerMessage?.contextInfo
      || null;
