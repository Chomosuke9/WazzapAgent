...
async function getChatName(ctx: AccountContext | undefined, chatId: string): Promise<string> {
...
async function handleMonitor({ chatId, account, sock, repos }: CommandContext): Promise<void> {
  try {
    ...
    for (const code of codes) {
      const typeLabel = code.type === 'all' ? 'all' : (code.type === 'group' ? 'group' : 'private');
    ...
    for (const act of activations) {
      ...
      activationLines.push(`${name} (${act.chatId}) | #${act.code} | Remaining: ${remaining}`);
    }
