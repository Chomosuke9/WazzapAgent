...
async function handleRevoke({
  ...
  if (isNaN(id) || id <= 0) {
    try {
      await sock.sendMessage(chatId, {
        text: "The ID must be a positive number. Use /monitor to see the list of IDs.",
      });
    } catch (err) {
      /* ignore */
    }
...
export const revokeCommand: CommandHandler = {
  commands: ["revoke"],
  description:
    "Revoke activation code made by /generate. see /monitor to see the list of activation code IDs.",
  permission: "owner",
