 ...
 * @param {string} lottiePayloadJson - JSON string of the lottieStickerMessage object
 *                            (as stored by addsticker.js → upsertLottieSticker)
 * @param {string} [replyTo] - Optional contextMsgId to reply to
 * @returns {Promise<{contextMsgId: string, messageId: string|null}>}
 */
async function sendLottieSticker(chatId, lottiePayloadJson, replyTo) {
  const sock = getSock();
  if (!sock) throw actionError('send_failed', 'WhatsApp socket not ready');
  if (!chatId) throw actionError('invalid_target', 'chatId is required');
  ...
  try {
    ...
    throw actionError('invalid_target', `Invalid Lottie payload JSON: ${err.message}`);
  }
  ...
  if (replyTo) {
    const quoted = resolveQuotedMessage(chatId, replyTo);
    if (quoted) {
