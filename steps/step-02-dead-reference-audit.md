...
## Acceptance criteria
- `grep -rn "commands" python/bridge/ | grep -vi "command_handler\|run_command\|slashCommand\|commandHandled\|# \|\"\"\""`
  shows no import of the deleted module.
- `python -c "import ast,sys; [ast.parse(open(f).read()) for f in sys.argv[1:]]" $(git ls-files 'python/**/*.py')`
