...
## Context
Phase 2 converts `src/*.js` to TypeScript one file at a time. For that to be
...
## Contract references
- None directly. Establishes the build that later compiles `src/protocol/types.ts`
  (CONTRACT.md §5) and the rest of the TS code.

## Files to read before starting
- `package.json`
- `scripts/patch-baileys.js` (referenced by `postinstall`)
- `src/index.js` (current entrypoint)
...
semantics; emit settings that overwrite `src/` in place.
...
- `pnpm dev` still boots the gateway from the existing `src/index.js` (reaches
