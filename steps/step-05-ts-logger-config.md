...
- Original - `migration/node/logger.js`
- `migration/node/config.js`
- `tsconfig.json`
...
## Files to modify
### `migration/node/logger.js` → `migration/node/logger.ts`
...
### `migration/node/config.js` → `migration/node/config.ts`
...
## Files to delete
- `migration/node/logger.js`, `migration/node/config.js` (replaced in place by `.ts`).
...
- `git ls-files migration/node/logger.* migration/node/config.*` lists only the `.ts` files.
