...
entrypoint/dispatcher. After this step the whole `src/` is TypeScript and behaves
...
`ActionAckPayload`/`WsErrorPayload` from `src/protocol/types.ts`. Behavior
unchanged; still `startWhatsApp()` + `wsClient.connect()`.

## Files to delete
- `src/wsClient.js`, `src/index.js`.

## Acceptance criteria
- `pnpm typecheck` passes with zero errors and `allowJs` is now effectively unused
  (no `.js` remains under `src/`: `git ls-files 'src/**/*.js'` is empty).
