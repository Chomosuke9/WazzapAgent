...
- `src/wsClient.ts`
- `git grep -n "wsClient" src/` (find all residual imports)
- `src/wa/connection.ts` (the `getSock()` shim from Step 17)
- `src/config.ts` (`wsEndpoint`/`wsToken` client fields)
...
## Files to delete
- `src/wsClient.ts` — unused after the flip.
...
## Acceptance criteria
- `git grep -n "wsClient" src/` returns **zero** matches.
