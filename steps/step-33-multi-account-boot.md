...
- Original - `migration/python/bridge/main.py` `main()` (Step 32 form)
- `migration/python/bridge/db.py` (current global DB-path resolution from env)
- `migration/python/bridge/session.py` (Step 32)
- `migration/node/server/wsServer.ts`, `migration/node/account/baileysFactory.ts` (confirm Node already
  creates per-folder accounts on connect, Steps 17/20)

## Files to create
### `migration/python/bridge/accounts.py` (config loader)
...
### `migration/node/wa/connection.ts` / `migration/node/account/*`
...
## Acceptance criteria
- `pytest migration/python/tests/test_accounts.py`: `load_accounts()` returns the configured
...
- `git grep -n "getSock" migration/node/` returns zero matches (Node fully multi-account).
- `pytest migration/python/tests/` and `node --test 'tests/node/**/*.test.mjs'` green.
